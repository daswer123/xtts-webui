...
with gr.Tab("Translate"):
             with gr.Row():
                with gr.Column():
                    ...
                    with gr.Column():
                        translate_whisper_model = gr.Dropdown(label="Whisper Model", choices=["small","medium","large-v2","large-v3"],value="medium")
                        translate_audio_mode = gr.Radio(label="Mode",choices=[1,2],value=1,info="1 - Takes each sentence as a sample and voices the text using this sample\n2 - Intended for 1 speaker, takes the sample that is longer and closest to the current sentence.")
                        translate_translator = gr.Radio(label="Translator",choices=["google","bing","baidu"],value="google")
                        with gr.Row():
                            translate_source_lang = gr.Text(value="auto",label="Enter lang code of source lang, if you want define automaticly type 'auto'")
                            translate_target_lang = gr.Dropdown(label="Target lang",choices=supported_languages_list,value="ru")
                            translate_speaker_lang = gr.Dropdown(label="Speaker Accent",choices=supported_languages_list,value="ru")
                with gr.Column():
